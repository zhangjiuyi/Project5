	...
	var app = new _vue2.default({
		...
		methods: {
			...
			signUp: function signUp() {
				...
				user.signUp().then(function (loginedUser) {
					...
					_this.currentUser = _this.formData.username;
					_this.user = loginedUser.attributes.username;
				}, function (error) {
					alert('注册失败');
				});
			},
		...
		created: function created() {
			...
			window.onbeforeunload = function () {
				...
				var todoString = JSON.stringify(_this3.newTodo);
				window.localStorage.setItem('newTodo', todoString);

				var userString = JSON.stringify(_this3.user);
